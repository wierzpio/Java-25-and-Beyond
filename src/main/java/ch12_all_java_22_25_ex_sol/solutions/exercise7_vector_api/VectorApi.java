package ch12_all_java_22_25_ex_sol.solutions.exercise7_vector_api;
...
public class VectorApi {
    void main() {
        ...
        float[] result3 = vectorComputation2(a, b);
        System.out.println("result using Vector API 2: " + Arrays.toString(result3));
    }
    ...
    static float[] vectorComputation(float[] a, float[] b) {
        ...
        for (; i < SPECIES.loopBound(a.length); i += SPECIES.length()) {
            ...
            var vc = va.mul(vb).add(va.mul(vb)).sub(va.add(vb));
            vc.intoArray(c, i);
        }
    ...
    static float[] vectorComputation2(float[] a, float[] b) {
        final VectorSpecies<Float> SPECIES = FloatVector.SPECIES_PREFERRED;
        final float[] c = new float[a.length];
        ...
        for (; i < SPECIES.loopBound(a.length); i += SPECIES.length()) {
            ...
            var vc = va.mul(vb).mul(2).sub(va.add(vb));
            vc.intoArray(c, i);
        }
